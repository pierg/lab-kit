...
3. **The lock is the bar.** Score a result against the commit-locked pre-registration, not against what the run seems to show. A verdict is bounded by its weakest instrument; an untested outcome is first-class; a miss is a miss. Check that no frozen surface named in `CLAUDE.md` was touched.
