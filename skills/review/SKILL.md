...
5. **Promote — the ladder move.** Ask of each result: *is this a citable fact?* If yes, add a row to `record/findings.md`:
   ...
   **The genre needs the right pin.** `ckit new story` works only on a content-kit pin at or after the commit that adds the `story` genre to `ckit/genres.json` — on an older pin it fails outright, and hand-declaring `story` under `genres` in `lab.json` does not substitute: `ckit/genres.py` copies an unknown genre's `checks` without validating them against what the pinned `lint.py` actually implements, so a hand-declared genre on an old pin routes to a directory but gets neither working check (`require_sections`, `bound_ids`) nor a skeleton to draft from. Re-vendor the kit (`make kit-sync`) instead of hand-declaring. The seal is mechanical, not only a convention: the `bound_ids` check requires the story's opening line — its first `<p class="sub">` — to carry at least one `<code>F-<n></code>` (a pinned cross-lab row, `<code>dsl:F-<n></code>`, counts too), and the gate refuses the page with "the opening line names no finding" if it does not.
