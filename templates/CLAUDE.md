# CLAUDE.md — operating guide for agents in <lab>
...
**If those imports did not expand for you, read `kit/DISCIPLINE.md` and `kit/LADDER.md` now — they are not optional context.** `DISCIPLINE.md` is the operating contract every lab inherits: append-only records, two git lanes, pre-register before you measure, every number re-derives, fail loud, frozen surfaces, the review gate. `LADDER.md` is where each kind of fact lives and how it is promoted.
